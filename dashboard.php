...
function sortMovies(&$movieArray, $sortType, $reviewCounts) {
    if (empty($movieArray)) return;
    usort($movieArray, function($a, $b) use ($sortType, $reviewCounts) {
        if ($sortType == 'asc') return strcmp($a['title'], $b['title']);
        if ($sortType == 'desc') return strcmp($b['title'], $a['title']);
        
        // --- UPDATED: 'pop' now sorts by TMDB Vote Average (Rating) ---
        if ($sortType == 'pop') return $b['vote_average'] <=> $a['vote_average'];
...
if (!empty($sort)) {
    ...
    sortMovies($search_results, $sort, $reviewCounts);
    sortMovies($all_movies, $sort, $reviewCounts);
}
?>
            ...
            ?>
                <div class="movie-card" 
                    ...
                    <div class="movie-info">
                        <h3><?= $movie['title'] ?></h3>
                        <p>⭐ Rating: <?= number_format($movie['vote_average'], 1) ?></p>
